...
**Startup recovery policy**: Profile activation still kicks off Home, Library, device discovery, and metadata sync immediately, but cold launches now also schedule bounded delayed recovery retries. If AAOS foregrounds the app before Bluetooth, the phone session, or Spotify's APIs are fully ready, `MainActivity.onResume()` and the delayed recovery job can re-run the initial loaders instead of leaving the session stranded after the first miss.

**Startup settlement policy**: Home, Library, and device bootstrap are now tracked by whether each loader successfully settled, not whether the resulting list is non-empty. That prevents valid empty states such as “no recent contexts yet” or “no devices visible right now” from triggering endless startup recovery loops. Home and Library also keep a short in-memory freshness window so navigating back to those screens does not immediately discard warm Room-backed data with another burst refresh.

**Playback-state error policy**: `SpotifyPlaybackController.getCurrentPlayback()` now rethrows transport-level failures such as `UnknownHostException`, `SocketTimeoutException`, and other I/O errors. The ViewModel keeps those failures distinct from Spotify's real `204 No Content` idle state, so a transient network problem no longer gets flattened into a false local "nothing is playing" result.
...
**Passive device registration + AVRCP wakeup**: When playback metadata reports an active device, `SpotifyViewModel` now forwards that device ID/name to `DeviceManager.registerActiveDevice()` so the last confirmed phone target stays cached. `DeviceManager` also preserves that remembered target across short `/devices` blind spots and can fall back to any active unrestricted Connect device when no phone is advertised. If a playback command returns a semantic failure, the ViewModel dispatches a native `KEYCODE_MEDIA_PLAY` down/up pair via Android `AudioManager`, waits 2 seconds, refreshes device discovery, and retries the command once; if Spotify still rejects the stale device ID with 404, `SpotifyPlaybackController` retries once more without `device_id` so Spotify can target the active session directly. Transport-level network failures now bypass the AVRCP wake-up path entirely and only mark the app offline, preventing dead-zone taps from waking the phone into unrelated local playback. The kickstart path is suppressed while AAOS is already in call, ringtone, or communication audio modes so Bluetooth HFP routing is left to the system stack.

**Mobile-network tuning**: Both OkHttp clients now use a shorter 10-second connect timeout with a 15-second read timeout and `retryOnConnectionFailure(true)`. That keeps the control path responsive during tower handoffs while still giving Spotify responses enough time to arrive on slow cellular links.

**Manifest role declaration**: Cloud-Bridge no longer declares `android:appCategory="audio"` or media foreground-service permissions. That keeps the app aligned with its remote-control architecture instead of advertising itself as a local AAOS media source.

**Profile bootstrap policy**: The old hard-coded legacy placeholder profile is no longer seeded at application start. If that placeholder row is still present from earlier builds, the app removes it during startup and clears profile-scoped cache/pins so the first real profile begins from clean state instead of inheriting stale library data.
